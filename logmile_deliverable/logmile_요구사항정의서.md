...
- 작성 기준일: 2026.05.15
- 변경 기준: 최상위 관리자/업체 관리자 구조, 업체 기반 멀티테넌시, PlateEvent, FE-BE API 정합성 검증, 시연용 시뮬레이션 직접 입력 흐름 반영
- 플랫폼: 웹 브라우저 기반 SPA
...
| ID | 기능 | 요구사항 | 담당 |
|---|---|---|---|
| FR-B01 | A/B/C 프리셋 선택 | 시뮬레이션 페이지에서 A 정상, B 주의, C 위험 프리셋을 선택하면 운행 시간, 휴식, 번호판 관측, 야간 운행 조건이 즉시 세팅되어야 한다. | 경서 |
| FR-B02 | GPS 시뮬레이션 실행 | 운행 시간, 휴식 패턴, 야간 운행 패턴이 포함된 GPS 데이터를 백엔드로 전송한다. 실제 GPS 기반 검증 흐름은 기존 구조를 유지한다. | 경서/환희 |
| FR-B03 | 시뮬레이션 시작/중지 | 실행 중인 시뮬레이션을 시작/중지하고 상태를 정리한다. 시연용 흐름에서는 운행 시작/종료 시각과 위치를 직접 입력할 수 있어야 한다. | 환희/경서 |
| FR-B04 | 진행 상태 표시 | 현재 시나리오, 경과 시간, 피로 점수, 피로 등급, 이벤트 로그를 표시한다. | 경서/환희 |
| FR-B05 | 시연용 번호판 입력 | 실제 환경의 번호판 자동 인식 결과를 시연 환경에서 사진 업로드 또는 번호판 직접 입력으로 대체할 수 있어야 한다. 입력 결과는 기존 `drive_log`, `plate_event` 구조에 맞춰 반영한다. | 경서/환희 |
| FR-B06 | 시연용 이벤트 추가 | 시뮬레이션 과정에서 입출차, 휴식, 운행 시간 관련 이벤트를 직접 추가하고 피로도 로직 변화를 확인할 수 있어야 한다. | 경서/환희 |
| FR-B07 | 기존 enum 유지 | 시연용 이벤트도 `ENTRY/EXIT`, `HIGHWAY_GATE/REST_AREA/CCTV`, `OCR/SIMULATOR/MANUAL/DUMMY` 등 기존 enum을 사용하며 enum 확장은 하지 않는다. | 환희/경서 |
...
| 시연용 시뮬레이션 | 페이지에서 A/B/C 프리셋을 선택하면 운행 시작/종료, 번호판 관측, 휴식, 운행 시간, 피로도 이벤트 흐름을 즉시 확인할 수 있어야 한다. |
