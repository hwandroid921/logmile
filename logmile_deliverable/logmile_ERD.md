...
| 번호판 관측 이벤트 | `plate_event` | 입출차, 고속도로 관문, 휴게소, CCTV 번호판 관측 이벤트 |
...
| `vehicle → plate_event` | 1:N | 차량 1대가 여러 번호판 관측 이벤트를 가질 수 있음. 미매칭 관측은 `vehicle_id = NULL` 가능 |
...
| 번호판 관측 이벤트 | `event_type = ENTRY/EXIT`, `location_type = HIGHWAY_GATE/REST_AREA/CCTV`, `source_type = OCR/SIMULATOR/MANUAL/DUMMY` |
...
|---|---|---|---|
| `company` | 10건 | 거의 없음 | 영구 |
| `admin` | 31건 | 거의 없음 | 영구 |
| `vehicle` | 50건 | 거의 없음 | 영구 |
| `driver` | 50건 | 거의 없음 | 영구 |
...
| `plate_event` | `idx_plate_event_plate_no` | `plate_no` | B-Tree | 번호판 문자열 기준 조회 |
