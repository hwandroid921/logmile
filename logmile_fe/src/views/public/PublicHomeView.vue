...
const scale = [
  { range: '0~39점',   label: '정상 (NORMAL)',  code: 'LEVEL_NORMAL_MAX=39',  bg: '#5E8A6F', text: '#FFFFFF' },
  { range: '40~69점',  label: '주의 (CAUTION)', code: 'LEVEL_CAUTION_MIN=40', bg: '#C58A3A', text: '#FFFFFF' },
  { range: '70~100점', label: '위험 (DANGER)',  code: 'LEVEL_DANGER_MIN=70',  bg: '#B5544A', text: '#FFFFFF' },
]
...
const features = [
  ...
  { num: '03', icon: 'eye',      title: '번호판 인식 매칭',     desc: '출발·고속도로 CCTV·휴게소 진출입·도착 — 4개 source_type으로 운행 로그를 자동 보강합니다.', tags: ['DEPARTURE / ARRIVAL', 'HIGHWAY / REST_AREA'], color: '#979EAE' },
  { num: '04', icon: 'bell',     title: '관리자 알림',          desc: '주의(40+) 시 운전자 알림, 위험(70+) 시 관리자 콘솔에 전화 권고가 활성화됩니다.', tags: ['CAUTION → 알림', 'DANGER → 전화'], color: '#B8BFC9' },
]
    ...
    <section class="hero-section">
      ...
      <div class="page-inner hero-body">
        ...
        <div class="hero-left">
          ...
          <!-- 피로 등급 -->
          <div>
            <div class="label-sm" style="margin-bottom:8px;">FATIGUE LEVELS · 0=안전 → 100=위험</div>
            <div class="scale-grid">
              <div v-for="(s,i) in scale" :key="i" :style="{padding:'14px',background:s.bg,color:s.text}">
                <div class="mono" style="font-size:9.5px;letter-spacing:0.12em;opacity:0.85;">{{ s.range }}</div>
                <div style="font-weight:700;font-size:14px;margin-top:6px;">{{ s.label }}</div>
                <div class="mono" style="font-size:9.5px;opacity:0.7;margin-top:4px;">{{ s.code }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style="position:relative;">
          <div class="card" style="padding:0;overflow:hidden;box-shadow:0 24px 60px rgba(81,95,122,0.18);">
            ...
            <div class="preview-bar">
              ...
              <div class="mono" style="flex:1;text-align:center;font-size:11px;color:var(--text-3);">logmile.io / dashboard / live</div>
              <AppIcon name="refresh" :size="12" style="color:var(--text-4);" />
            </div>
            <!-- KPI -->
            <div style="display:grid;grid-template-columns:repeat(3,1fr);border-bottom:1px solid var(--line-1);">
              <div v-for="(k,i) in heroKpi" :key="k.l" :style="{padding:'14px 16px',borderRight:i<2?'1px solid var(--line-1)':'none'}">
                ...
                <div style="display:flex;align-items:baseline;gap:8px;margin-top:6px;">
                  ...
                  <span class="mono" style="font-size:11px;color:var(--text-4);">{{ k.d }}</span>
                </div>
              </div>
            </div>
            <!-- 차량 목록 -->
            <div style="padding:14px;">
              <div class="label-sm" style="margin-bottom:10px;display:flex;justify-content:space-between;">
                ...
                <span style="color:var(--danger);">● LIVE</span>
              </div>
              <div style="display:flex;flex-direction:column;gap:8px;">
                <div v-for="r in heroRows" :key="r.plate"
                  ...
                  <div style="width:36px;height:24px;background:var(--bg-3);border:1px solid var(--line-2);border-radius:3px;display:flex;align-items:center;justify-content:center;">
                    <span class="mono" style="font-size:8px;color:var(--text-4);">KR</span>
                  </div>
                  <div>
                    <div class="mono" style="font-size:12px;font-weight:700;">{{ r.plate }}</div>
                    <div style="font-size:11px;color:var(--text-3);margin-top:2px;">{{ r.driver }} · {{ r.loc }}</div>
                  </div>
          ...
          <div style="position:absolute;top:-14px;right:-14px;background:var(--bg-1);border:1px solid var(--accent-line);padding:8px 12px;border-radius:6px;display:flex;align-items:center;gap:8px;box-shadow:0 8px 20px rgba(81,95,122,0.18);">
            ...
            <span class="mono" style="font-size:11px;color:var(--text-1);font-weight:700;">2건 위험 (70+)</span>
          </div>
        </div>
      </div>
      ...
      <div class="page-inner">
        <div class="hero-stats-bar">
          <div v-for="(s,i) in heroStats" :key="s.l"
            ...
            <div class="mono" style="font-size:28px;font-weight:800;color:var(--accent);letter-spacing:-0.01em;">{{ s.v }}</div>
            <div class="label-sm" style="margin-top:6px;">{{ s.l }}</div>
            <div style="font-size:11.5px;color:var(--text-3);margin-top:4px;">{{ s.s }}</div>
          </div>
        </div>
      </div>
    </section>
        ...
        <div class="feat-grid">
          <div v-for="f in features" :key="f.num" class="card feat-card">
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px;">
              ...
              <span class="mono" style="font-size:11px;color:var(--text-4);">{{ f.num }}</span>
            </div>
            <h3 style="font-size:15.5px;margin:0 0 8px;font-weight:700;">{{ f.title }}</h3>
            <p style="font-size:12.5px;color:var(--text-3);line-height:1.65;margin:0;">{{ f.desc }}</p>
            <div class="feat-tags">
    ...
    <section class="section-pad section-border" style="background:var(--bg-0);">
      <div class="page-inner">
        ...
        <div class="card" style="overflow:hidden;">
          <div class="wide-grid">
            ...
            <div style="padding:20px;border-right:1px solid var(--line-1);">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
                <div>
                  <div class="label-sm">RUNNING FLEET · 5 LATEST</div>
                ...
                <div v-for="(r,i) in wideRows" :key="r.plate"
                  ...
                  <div>
                    <div style="display:flex;align-items:center;gap:6px;">
                      <span class="mono" style="font-size:12px;font-weight:700;">{{ r.plate }}</span>
                      <span class="chip chip-mute" style="font-size:9.5px;padding:1px 5px;">{{ r.scenario }}</span>
                    </div>
                  ...
                  <span class="mono" style="font-size:11px;color:var(--text-4);text-align:right;">실시간</span>
                </div>
              </div>
            </div>
            <!-- 트렌드 차트 -->
            <div style="padding:20px;">
              <div class="label-sm" style="margin-bottom:6px;">FATIGUE TREND · LAST 14 DAYS</div>
              <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:16px;">
                <span class="mono" style="font-size:30px;font-weight:800;letter-spacing:-0.015em;">53.7</span>
                <span class="mono" style="font-size:12px;color:var(--danger);">▲ 5.2</span>
                <span style="font-size:11px;color:var(--text-4);">vs 지난주</span>
              </div>
          ...
          <div style="padding:10px 20px;background:var(--bg-2);display:flex;justify-content:space-between;font-size:11px;font-family:var(--font-mono);color:var(--text-3);">
      ...
      <div class="page-inner">
        ...
        <div class="steps-grid">
          <div v-for="(s,i) in steps" :key="s.n" class="card" style="padding:24px;position:relative;">
            <div class="mono" style="font-size:10.5px;color:var(--accent);letter-spacing:0.14em;">STEP {{ s.n }}</div>
            ...
            <div style="font-size:12.5px;color:var(--text-3);line-height:1.65;">{{ s.d }}</div>
            <div v-if="i<3" style="position:absolute;right:-9px;top:50%;transform:translateY(-50%);background:var(--bg-1);padding:0 4px;color:var(--text-4);">
    ...
    <section class="section-pad">
      <div class="page-inner">
        <div class="card cta-card">
          <div>
            <div class="label-sm" style="color:var(--accent);margin-bottom:10px;">FEATURE DEMO</div>
            <h2 style="font-size:30px;margin:0 0 8px;font-weight:800;letter-spacing:-0.022em;">기능 미리보기</h2>
...
.label-sm {
  ...
  font-size: 10.5px; letter-spacing: 0.12em; text-transform: uppercase; color: var(--text-4);
}
.mono { font-family: var(--font-mono); }

/* ── 섹션 헤더 ── */
.section-hd { margin-bottom: 36px; }
.eyebrow { font-family: var(--font-mono); font-size: 10.5px; letter-spacing: 0.12em; text-transform: uppercase; color: var(--accent); margin-bottom: 8px; }
.section-title { font-size: 28px; font-weight: 800; margin: 0 0 10px; letter-spacing: -0.022em; color: var(--text-1); }
.section-sub { font-size: 13.5px; color: var(--text-3); line-height: 1.7; margin: 0; max-width: 640px; }
...
.hero-title { font-size: 48px; font-weight: 800; line-height: 1.1; letter-spacing: -0.026em; margin: 0; color: var(--text-1); }
...
/* ── Buttons ── */
.btn { display: inline-flex; align-items: center; gap: 7px; padding: 9px 18px; border-radius: var(--r-md); font-size: 13.5px; font-weight: 600; text-decoration: none; cursor: pointer; transition: background 0.15s, border-color 0.15s, color 0.15s; }
...
/* ── Chips / Dots ── */
.chip { display: inline-flex; align-items: center; gap: 5px; padding: 3px 9px; border-radius: 999px; font-size: 11.5px; font-weight: 500; }
